...
[Clique aqui para acessar o Dashboard Interativo]([url](https://lookerstudio.google.com/u/0/reporting/3f5e8dde-6737-45af-8922-31273f9de921/page/p_2jccde8nxd))
...
Cálculo de Idade Estimada (Tabela slv_ecommerce_users):
...
Cálculo de Margem Agregada (Tabela gld_ecommerce_top_produtos):
...
Correção de NULLs na Camada Gold (Tabela gld_ecommerce_fato_vendas):
...
Investigação: A causa raiz foi um LEFT JOIN na fato_vendas que mantinha itens de pedidos Cancelled (que foram filtrados da slv_orders), gerando NULLs em todas as colunas de data.
...
Enriquecimento da Tabela de Fatos (Tabela gld_ecommerce_fato_vendas):

Desafio: O time de performance precisa de segmentações que exigem JOINs complexos na ferramenta de BI, causando lentidão.
...
Otimização de BI (Tabela gld_ecommerce_rfm_analise_categoria):
...
Garanta que as permissões do Agente de Serviço do Dataform (service-<ID>@gcp-sa-dataform.iam.gserviceaccount.com) tenham os papéis de "Editor de Dados do BigQuery" e "Usuário de Job do BigQuery" no IAM.

No ambiente Dataform, clique no botão principal "Start Execution".

Selecione a opção "All actions".

Clique em "Start execution". O Dataform analisará o grafo de dependências e executará todas as tabelas na ordem correta (Bronze → Silver → Gold).
...
Insight de Qualidade de Dados (Ação Imediata):
...
Insight de Segmentação (Onde está o Valor):
...
Insight de Risco (Recuperação de Clientes):
